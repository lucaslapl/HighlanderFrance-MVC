...
/**
 * Mise à jour des stats de la page d'accueil (update_index_stats.php).
 */
final class UpdateIndexStatsService
{
    ...
    private const CACHE_FILE = DATA_DIR . '/cache_hlfr_stats.json';
    private const HISTORY_FILE = DATA_DIR . '/log_update_index_stats.txt';
    ...
    public function run(): string
    {
        ...
        $blacklistRepo = new MatchLogRepository($this->db);
        ...
        $shortLogIds = $this->db->query('SELECT match_id FROM matches_cache WHERE length > 0 AND length < ' . MIN_MATCH_LENGTH)
        ...
        file_put_contents(self::CACHE_FILE, json_encode($result));
