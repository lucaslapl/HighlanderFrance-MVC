 ...
 * /api/server/live-status ; ce service écrit un cache JSON (répertoire _scripts,
 * protégé par .htaccess) et expose les matchs "frais" via /api/live-matches.
 */
final class LiveMatches
{
    public const FILE = DATA_DIR . '/live_matches.json';
    ...
    public static function enrich(array $entry): array
    {
        ...
        if ($steamid3s !== []) {
            $placeholders = implode(',', array_fill(0, count($steamid3s), '?'));
            $stmt = Database::connection()->prepare("
                SELECT steamid, name, display_name, avatar
                FROM players_info
                WHERE steamid IN ($placeholders)
            ");
            $stmt->execute($steamid3s);

            foreach ($stmt->fetchAll(\PDO::FETCH_ASSOC) as $row) {
                $known[$row['steamid']] = $row;
            }
        }
